...
The Agents component monitors open local AI sessions from Codex, Google Antigravity, Cursor, and Claude (Claude.app plus Claude Code sessions started in a terminal, read from `~/.claude/sessions`) through a pluggable `AgentToolFactory` / `AgentToolInterface` layer (`NotchApp/Features/Agents/Interfaces/`). Each tool owns a native status enum (mapped to shared `working` / `waitingForUser` / `completed` buckets), a provider adapter, and a filesystem signal monitor that triggers resync so counters and the Dynamic Island stay current. Adding another AI product means registering a new interface in the factory rather than special-casing `AgentMonitorStore`. Each application has its own icon and three live counters: blue for working agents, orange for agents waiting on user approval or a decision, and green for completed agents. Dynamic Island priority is orange → blue → green. In narrow cards application names are hidden (for all rows at once) and counters drop their symbols before anything is truncated. The view refreshes about every second (and on SQLite/WAL file changes) and shows zeroes when its source application is not running. Settings → Agenci AI chooses which agents appear; disabled agents are not monitored and do not reach the Dynamic Island.

The expanded surface has three tabs: `Notch` for configured components, `Tray` for persistent file drops, and `Photos` for screenshots. Dropped files and folders are copied to `~/Library/Application Support/com.notchnook.app/Tray`, indexed in JSON, restored after relaunch, and can be removed individually from the Tray UI.

While `Tray` or `Photos` is selected, outside clicks intentionally keep the panel open so files can be collected from or dragged to other applications. A dedicated close button appears below Settings in this mode.

`Photos` lists images tagged by macOS as screen captures (`kMDItemIsScreenCapture`) in the system screenshot folder, newest first. Settings → Zrzuty ekranu changes that folder by writing the system `com.apple.screencapture` `location` preference, the same one the Screenshot app (⇧⌘5) edits. The regular `Notch` tab retains outside-click dismissal.
