...
The Agents component monitors open local AI sessions from Codex, Google Antigravity, and Cursor. Each application has its own icon and three live counters: blue for working agents, orange for stopped or waiting agents, and green for completed agents. The view refreshes every two seconds and shows zeroes when its source application is not running.
