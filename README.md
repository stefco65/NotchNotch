...
The Agents component monitors open local AI sessions from Codex, Google Antigravity, and Cursor through a pluggable `AgentToolInterface` factory. Each tool owns a native status enum and a filesystem signal monitor that refreshes counters and the Dynamic Island. Each application has its own icon and three live counters: blue for working agents, orange for agents waiting on user approval or a decision, and green for completed agents. Dynamic Island priority is orange → blue → green. The view refreshes about every second (and on SQLite/WAL file changes) and shows zeroes when its source application is not running.
